...
{
    Ticket *new_rear = NULL;
    new_rear = (Ticket *)calloc(1, sizeof(Ticket));
    if (new_rear == NULL || data == NULL) return false;
    strcpy(new_rear -> name, name);
    if (data -> last == NULL)
    {
        data -> last = new_rear;
...
/**
 * Gets a name to from the front of the Queue.
 *
...
{
    Ticket *new_top = NULL;
    new_top = (Ticket *)calloc(1, sizeof(Ticket));
    if (new_top == NULL || data == NULL) return false;
    ...
    {
        new_top -> next = data -> first -> next;
        data -> first -> next = new_top;
    }
    return true;
}

/**
 * Gets a name to from the top of the Stack.
 *
